...
class MatplotlibGraphPlot(BaseGraphPlot):
    ...
    def _plot_structure_nodes(graph: StructureGraph, ax=None) -> None:
        ...
        for node in graph.nodes:
            if graph.nodes[node]["type"] == "root":
                color = cmap(0.2)
            elif graph.nodes[node]["type"] == "directory":
                color = cmap(0.5)
            else:
                color = cmap(0.8)
            text_box = ax.text(
        ...
        fig.canvas.draw()
        height = None
        for text_box, node in zip(text_boxes, graph.nodes):
            ...
            graph.nodes[node]["width"], graph.nodes[node]["height"] = (
                extents[1, 0] - extents[0, 0],
                height,
            )
        x, y = zip(
            *[(graph.nodes[node]["x"], graph.nodes[node]["y"]) for node in graph.nodes]
        )
        min_x, max_x = min(x), max(x)
        min_y = min(y)
        for node in graph.nodes:
            graph.nodes[node]["x"] = (graph.nodes[node]["x"] - min_x) / (max_x - min_x)
            graph.nodes[node]["y"] = -graph.nodes[node]["y"] / min_y + 1
        for text_box, node in zip(text_boxes, graph.nodes):
            text_box.set_position((graph.nodes[node]["x"], graph.nodes[node]["y"]))
        ax.set_xlim((0.0, 1.0))
        ax.set_ylim((-0.1, 1.1))
